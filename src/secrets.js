...
(function (exports, global) {
  ...
  function init (bits) {
    ...
    const logs = []; const exps = []; let x = 1; const primitive = defaults.primitivePolynomials[config.bits]
  ...
  function getRNG () {
    ...
    if (global.crypto && typeof global.crypto.getRandomValues === 'function' && typeof global.Uint32Array === 'function') {
      ...
      return function (bits) {
        ...
        let str = null
        const arr = new global.Uint32Array(elems)
  ...
  exports.share = function (secret, numShares, threshold, padLength, withoutPrefix) {
    ...
    const x = new Array(numShares); const y = new Array(numShares)
  ...
  function processShare (share) {
    ...
    const id = parseInt(share.substr(1, idLength), config.radix)
  ...
  function combine (at, shares) {
    let setBits; let share; const x = []; const y = []; let result = ''; var idx
  ...
  exports.newShare = function (id, shares) {
    ...
    const share = processShare(shares[0])
    ...
    const padding = max.toString(config.radix).length
